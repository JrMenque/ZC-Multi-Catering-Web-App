...
----------------------------------------------*/
	...
	public function delete_user($id)
	{
		...
		$userArray = ['users_active_features', 'user_settings', 'allergens', 'menu_type'];
		$del = $this->deleteByUserID($id, $userArray);

		$shopTables = ['users_active_order_types', 'items', 'item_list', 'coupon_list', 'delivery_area_list', 'expense_category_list', 'expense_list', 'extra_libraries', 'hotel_list', 'item_category_list', 'item_packages', 'order_item_list', 'order_user_list'];
		$del = $this->deleteByShopID($shop_id, $shopTables);
	...
	protected function deleteByShopID($shop_id, $tables)
	{
		foreach ($tables as $row) {
			$this->admin_m->delete_all_by_vendor_id($shop_id, $row);
		}
	...
	{
		foreach ($tables as $row) {
			$this->admin_m->delete_all_by_user_id($user_id, $row);
		}
