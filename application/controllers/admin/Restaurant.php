...
----------------------------------------------*/
	...
	public function order_status($id, $status)
	{
		...
		$this->version_changes_m->pusher(restaurant()->id, 'order_status'); //pushr for order status
		$this->user_email_m->send_order_mail($order_info['uid'], $status); //send mail when order accept,completed and reject
...
================================================== */
	...
	public function delete_order_items($uid, $id)
	{
		...
		if ($delete) :
			...
			//calculate form item_list 
			$previous_price = $this->admin_m->get_total_price_by_order_id($order_info['id']);
			$present_price = $previous_price;
			$price_format = number_format((float)$present_price, 2, '.', '');

			$total_price = grand_total($price_format, $order_info['delivery_charge'], $order_info['discount'], $order_info['tax_fee'], $order_info['coupon_percent'], $order_info['tips'], $order_info['order_type'], restaurant()->tax_status, $order_info['is_pos'], $order_info['is_item_tax'], $order_info['service_charge']);
	...
	public function edit_order($order_id)
	{
		...
		foreach ($item_details as $key => $item) {
			...
			$cart_data = array(
				...
				'tax_status' => !empty($item['tax_status']) ? $item['tax_status'] : 0,
