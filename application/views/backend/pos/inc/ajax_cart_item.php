    ...
    foreach ($this->cart->contents() as $key => $row) : ?>
        <?php if (isset($row['is_pos']) && $row['is_pos'] == 1) : ?>
            ...
            if (isset($i_info['service_charge']) && !empty($i_info['service_charge'])) {
                ...
                $service_charge = __service_charge($subtotal, restaurant()->id)->price ?? 0;
            }
                    ...
                    <div class="cartitemDetails">
                        ...
                        <div class="itemPriceArea">
                            ...
                            <?php if (isset($row['sizes']['is_size']) && $row['sizes']['is_size'] == 1) : ?>
                                <p class="subTotalTag">
                                    <?= currency_position(($row['qty'] * $row['item_price']), $shop_info->id); ?></p>
                            <?php else : ?>
    ...
    <div class="cartOrderInfo">
        ...
        <div class="cartActionArea">
            <ul>
                ...
                <li><a href="#shippingModal" <?= empty(auth('is_order_edit')) ? 'data-toggle="modal"' : ""; ?>><span><i class="icofont-fast-delivery"></i><?= !empty($shipping) ? currency_position($shipping, $shop_info->id) : "";; ?></span>
