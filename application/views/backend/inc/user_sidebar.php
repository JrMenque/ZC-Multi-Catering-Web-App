...
<?php if ($this->auth['is_payment'] == 1 && $this->auth['is_expired'] == 0 && shop_id() > 0) : ?>
    ...
    <?php if (file_exists(APPPATH . 'controllers/admin/Pos.php')) : ?>
        <?php if (is_feature(auth('id'), 'pos') == 1) : ?>
             <li class="treeview pos <?= isset($page) && $page=="POS"?"active":"";?> <?= isset($page_title) && $page_title=="Expenses"?"active":"";?>">
                <a href="#">
                ...
                <ul class="treeview-menu">
                    <?php if (is_access('pos-order') == 1) : ?>
                       <li class="<?= $page_title =="POS"?"active":"";?>"><a href="<?= base_url('admin/pos') ?>"><i class="fa fa-plus"></i> <?= !empty(lang('add_new_order'))?lang('add_new_order'):"Add New Order";?></a></li>
                    <?php endif; ?>

                    <?php if (is_access('pos-settings') == 1) : ?>
                        <li class="<?= $page_title =="POS Settings"?"active":"";?>"><a href="<?= base_url('admin/pos/settings') ?>"><i class="fa fa-angle-double-right"></i> <?= !empty(lang('settings'))?lang('settings'):"Settings";?></a></li>
