    ...
    <div class="homeTopMenu">
        <ul class="left_top_menu">
            ...
            <?php if(!empty($home_setting['phone'])): ?>
                <li><a href="<?= redirect_url(html_escape($home_setting['phone']),'phone') ;?>"><i
        ...
        <ul class="right_top_menu">
            ...
            <?php if(!empty($home_setting['instagram'])): ?>
                <li><a href="<?= redirect_url($home_setting['instagram'],'instagram');?>"><i
        ...
        <nav class="navbar navbar-expand-lg navbar-light bg-light homeMenuNav">
            <div class="container">
                <a class="navbar-brand" href="<?= base_url(); ?>"><img
                    src="<?= base_url(!empty($settings['logo'])?$settings['logo']:"assets/frontend/images/logo-example.png"); ?>"
                    class="logo" alt=""></a>
                    ...
                    <div class="custom_menu">
                        <ul class="navbar-nav">
                            <li class="nav-item <?= isset($page_title) && $page_title=='Home'?'active':''; ?>">
                                <a class="nav-link"
                                href="<?= base_url($ref_id); ?>"><?= !empty(html_escape(lang('home')))?html_escape(lang('home')):"Home";?>
                            ...
                            <li class="nav-item <?= isset($page_title) && $page_title=='Pricing'?'active':''; ?>">
                                <a class="nav-link"
                                href="<?= base_url('pricing').$ref_id; ?>"><?= !empty(html_escape(lang('pricing')))?html_escape(lang('pricing')):"pricing";?></a>
                            </li>
                        <?php endif;?>
                        ...
                        <?php if($this->common_m->total_rating() > 0): ?>
                            <li class="nav-item <?= isset($page_title) && $page_title=='Reviews'?'active':''; ?> hidden">
                                <a class="nav-link"
                                href="<?= base_url('reviews'); ?>"><?= !empty(html_escape(lang('reviews')))?html_escape(lang('reviews')):"reviews";?></a>
                            </li>
                        <?php endif;?>
                    <?php endif;?>
                    ...
                    <?php if($settings['is_user']==1): ?>
                        <?php if($this->common_m->count_total_user() > 0): ?>
                            <li class="nav-item <?= isset($page_title) && $page_title=='Users'?'active':''; ?>">
                                <a class="nav-link"
                                href="<?= base_url('users'); ?>"><?= !empty(html_escape(lang('users')))?html_escape(lang('users')):"users";?></a>
                            </li>
                        <?php endif;?>
                    <?php endif;?>
                ...
                <li class="nav-item <?= isset($page_title) && $page_title=='Contacts'?'active':''; ?>">
                    <a class="nav-link"
                    href="<?= base_url('contacts').$ref_id; ?>"><?= !empty(html_escape(lang('contacts')))?html_escape(lang('contacts')):"contacts";?></a>
                </li>
            <?php endif;?>
                ...
                <?php if(isset($settings['is_lang_list']) && $settings['is_lang_list']==1): ?>
                   ...
                   <?php if(sizeof($languages) > 1): ?>
                    <li class="nav-item dropdown">
                        ...
                        <?= !empty(auth('site_lang'))?auth('site_lang'):$settings['language'] ;?>
                    </a>
                    <div class="dropdown-menu languageDropdown" aria-labelledby="navbarDropdown">
                     
                        <?php foreach ($languages as $key => $language): ?>
                            <a class="dropdown-item"
                            href="<?= base_url('home/lang_switch/'.$language['slug']).$ref_id ;?>"><?= $language['lang_name'] ;?></a>
                        <?php endforeach ?>
