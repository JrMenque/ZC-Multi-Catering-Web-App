					...
					<?php if (shop($row['shop_id'])->is_cart == 1) : ?>
						...
						<?php if (shop($row['shop_id'])->stock_status == 1) : ?>
							...
							<?php if ($row['in_stock'] > $row['remaining']) : ?>
								<a href="javascript:;" class="<?= (isset($extra->is_extra) && $extra->is_extra == 1) || $row['is_size'] == 1 ? "quick_view" : "add_to_cart"; ?>" data-id="<?= html_escape($row['item_id']); ?>" data-type="item" data-placement="top" data-toggle="" title="Add to Cart"><i class="icofont-ui-cart"></i> <span> <?= lang('order_now'); ?></span></a>
							<?php endif; ?>
