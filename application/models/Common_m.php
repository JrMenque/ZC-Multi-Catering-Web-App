...
class Common_m extends CI_Model
{
	...
	{
		$this->db->select('oi.*');
		$this->db->select('i.id as items_id,i.title as name,i.thumb as item_thumb,i.cat_id,i.allergen_id,i.veg_type,i.price,a.name as allergen,i.tax_fee,i.tax_status');
		$this->db->select('ip.package_name,ip.thumb as package_thumb,ip.final_price');
		$this->db->from('order_item_list oi');
		$this->db->join('items i', 'i.id = oi.item_id', 'LEFT');
		$this->db->join('allergens a', 'a.id = i.allergen_id', 'LEFT');
	...
	{
		$this->db->select('l.*,r.username as shop_username,r.name as shop_name,r.thumb,r.slogan,u.qr_link');
