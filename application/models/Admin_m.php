...
public function check_pass($pass)
{
    ...
    public function get_time_config($id)
    {
        ...
        if ($query->num_rows() > 0) :
            $data =  $query->row();
            return isJson($data->time_config) ? json_decode($data->time_config) : $data->time_config;
        else :
            return '';
        endif;
    }
...
{
    $this->db->select('ol.*,c.city_name,c.zip_code');
    $this->db->from('staff_list ol');
...
public function get_all_delivery_order_list($user_id, $status, $is_picked)
{
    ...
    $today = today();
    $this->db->select('or.*, sl.name as staff_name, sl.phone as staff_phone, sl.country_id, rl.name as shop_name, sl.city_id as staff_city_id, rl.city_id as restaurant_city_id');
    $this->db->from('order_user_list or');
    $this->db->join('staff_list sl', 'sl.id = or.customer_id', 'LEFT');
    $this->db->join('restaurant_list rl', 'rl.id = or.shop_id AND rl.city_id = sl.city_id', 'LEFT');
    $this->db->where('or.status', 2);
...
public function get_staff_activities($uid, $role = 'staff')
{
    ...
    if (!empty($_GET['daterange'])) {
        ...
        $sql = "(DATE_FORMAT(u.created_at,'%Y-%m-%d') BETWEEN '{$newDate1}' AND '{$newDate2}')";
        $this->db->where($sql);
    }
...
public function get_my_addons($item_id, $shop_id)
{
    $this->db->select('e.*');
