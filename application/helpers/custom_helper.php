        ...
        if ($order_type == 1) {
            if ($is_item_tax == 1) {
                ...
                elseif ($tax_status == '--') :
                    ...
                    $total = ($amount + $shipping + $tips + $service_charge) - ($discount + $coupon_percent + $tax);
                endif;
            }
        } else {
            ...
            if ($is_item_tax == 1) {
                ...
                elseif ($tax_status == '--') :
                    ...
                    $total = ($amount + $tips + $service_charge) - ($discount + $coupon_percent + $tax);
                endif;
            }
        }
...
if (!function_exists('get_total')) {
    function get_total($amount = 0, $shipping = 0, $discount = 0, $tax = 0, $coupon_discount = 0, $tips = 0, $tax_status = '+', $service_charge = 0)
    {
        ...
        if ($tax_status == "+") {
            ...
            return ($amount + $shipping + $tips + $service_charge) - ($discount + $coupon_discount + $tax);
        }
    }
}
...
if (!function_exists('orderId')) {
    ...
    {
        $ci = &get_instance();
        return date('Y') . random_string('numeric', 2) . $shopId . random_string('numeric', 2);
    }
}
...
if (!function_exists('imgRation')) {
    function imgRation($url)
    {
        ...
        $ratio = 'square';
        if (!empty($url)) :
            list($height, $width) = getimagesize(base_url($url));
            if ($width > $height) {
                $ratio = 'landscape';
